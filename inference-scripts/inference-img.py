...
class _DotDict:
    ...
    @classmethod
    def _wrap(cls, v):
        """Recursively wrap dicts; keep lists as lists but wrap their dict elements."""
    ...
    # Allow dict-style .get() so safe_get() and extract_normalize_params work uniformly
...
def load_config(cfg_obj: _DotDict, path: str) -> None:
    """Load a NanoDet YAML config file into *cfg_obj* in-place."""
...
# Global config object — populated by load_config() in main()
cfg = _DotDict({})
...
def parse_args():
    parser = argparse.ArgumentParser(description="NanoDet NCNN RTSP inference")
    ...
    parser.add_argument("--image", default=None, help="path to a single image for testing (skips RTSP)")
    ...
    parser.add_argument(
        "--save_dir",
        default=None,
        help="optional directory to save annotated frames periodically",
    )
    parser.add_argument(
        "--save_interval",
        type=float,
        default=5.0,
        help="seconds between saved snapshots",
    )
    parser.add_argument(
        "--reconnect_delay",
        type=float,
        default=3.0,
        help="seconds to wait before reconnecting RTSP stream",
    )
    parser.add_argument(
        "--quiet",
        action="store_true",
        help="disable per-frame console logging",
    )
    parser.add_argument(
        "--apply_sigmoid",
        action="store_true",
        help="apply sigmoid to class scores manually; usually not needed for exported NanoDet models",
    )
    parser.add_argument(
        "--violation_duration",
        type=float,
        default=3.0,
        help="seconds a forklift must remain outside the safe zone before alarm triggers (default: 3.0)",
    )
...
def parse_ncnn_param_io(param_path):
    """
    Parse NCNN .param file and detect:
      - input blob(s)
      - terminal output blob(s) = produced but never consumed
    """
    ...
    # Skip magic/header lines if present
    start_idx = 0
...
# ---------------------------------------------------------------------------
# Safe zone + exclusion zone definitions
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Ground point extraction + zone check
# ---------------------------------------------------------------------------

def get_ground_points(dets, score_thres):
    """
    For every detection above score_thres, compute the ground contact point
...
def check_zone(ground_point, safe_zone, exclusion_zone):
    """
    Classify a ground point against the safe zone and exclusion zone.

    Returns one of three strings:
        "safe"      — inside the pathway polygon
        "excluded"  — outside pathway but inside exclusion zone (repair bay)
        "violation" — outside pathway AND outside exclusion zone → real violation
    """
    import numpy as np
...
def _pipeline_to_list(pipeline):
    """
    Normalise a pipeline value to a flat list of step objects.

    NanoDet configs define pipeline either as:
      - a YAML sequence  → Python list   (most common)
      - a YAML mapping   → _DotDict      (some custom configs)
    In both cases we want a list of step objects to iterate over.
    """
    if pipeline is None:
        return []
...
def extract_normalize_params(cfg_obj):
    ...
    for step in pipelines:
        # Accept both plain dicts and _DotDict nodes (both expose .get())
...
class NCNNPredictor:
    def __init__(
        ...
        self.reg_max = safe_get(cfg_obj, "model.arch.head.reg_max", None)
        if self.reg_max is None:
            self.reg_max = safe_get(cfg_obj, "model.head.reg_max", 7)
        self.reg_max = int(self.reg_max)

        self.strides = safe_get(cfg_obj, "model.arch.head.strides", None)
        if self.strides is None:
            self.strides = safe_get(cfg_obj, "model.head.strides", [8, 16, 32, 64])
        ...
        self.logger.log(
            f"NCNN loaded | input_blob={self.input_blob} | output_blobs={self.output_blobs} "
            f"| input_size=({self.input_w},{self.input_h}) | reg_max={self.reg_max} | strides={self.strides}"
        )
    ...
    def _reshape_single_output(self, arr):
        ...
        if arr.ndim == 2:
            ...
            if flat.size % expected_dim != 0:
                raise RuntimeError(
                    f"Unexpected 2D output shape {arr.shape}, expected one axis to equal {expected_dim}"
                )
            return flat.reshape(-1, expected_dim)

        if arr.ndim == 3:
            # Accept CHW or HWC-like cases if one axis matches expected_dim
            if arr.shape[-1] == expected_dim:
                return arr.reshape(-1, expected_dim)
            if arr.shape[0] == expected_dim:
    ...
    def postprocess(self, pred, meta):
        ...
        if num_points != expected_points:
            raise RuntimeError(
                ...
                f"Check output blobs / input size / strides."
            )
...
def main():
    ...
    # ----------------------------------------------------------------
    # IMAGE MODE — run on a single file, show result, then exit
    # ----------------------------------------------------------------
    if args.image:
        ...
        for fk in forklift_data:
            fk["status"] = check_zone(fk["ground_point"], SAFE_ZONE, EXCLUSION_ZONE)

        import numpy as np
        out_frame = frame.copy()
        cv2.polylines(out_frame, [np.array(SAFE_ZONE, dtype=np.int32)],
                      isClosed=True, color=(0, 255, 0), thickness=2)
        cv2.polylines(out_frame, [np.array(EXCLUSION_ZONE, dtype=np.int32)],
                      isClosed=True, color=(255, 165, 0), thickness=2)
        ...
        for fk in forklift_data:
            ...
            cv2.putText(out_frame, label, (x1, max(20, y1 - 8)),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 2)
            cv2.circle(out_frame, (gx, gy), 6, color, -1)
            logger.log(f"ground_point={fk['ground_point']}  status={status}")

        if not forklift_data:
            logger.log("No forklifts detected.")

        cv2.imshow("Image Test", out_frame)
        logger.log("Press any key to close.")
        cv2.waitKey(0)
        ...
        if args.save_dir:
            ...
            out_path = _os.path.join(args.save_dir, _os.path.basename(args.image))
            cv2.imwrite(out_path, out_frame)
            logger.log(f"Saved to {out_path}")
        return

    if not args.rtsp:
        raise RuntimeError("Provide either --rtsp or --image.")

    cap = open_stream(args.rtsp)
    window_name = "NanoDet NCNN Detection"
    ...
    try:
        while True:
            ...
            # --- Ground point extraction + zone classification ---
            forklift_data = get_ground_points(dets, args.score_thres)
            for fk in forklift_data:
                fk["status"] = check_zone(fk["ground_point"], SAFE_ZONE, EXCLUSION_ZONE)
            ...
            if needs_rendered:
                # Draw safe zone and exclusion zone polygons on frame
                out_frame = frame.copy()
                import numpy as np
                cv2.polylines(out_frame, [np.array(SAFE_ZONE, dtype=np.int32)],
                              isClosed=True, color=(0, 255, 0), thickness=2)
                ...
                # Draw each forklift with color based on status
                for fk in forklift_data:
                    x1, y1, x2, y2 = fk["box"]
                    gx, gy         = fk["ground_point"]
                    status         = fk["status"]
                    ...
                    cv2.putText(out_frame, label, (x1, max(20, y1 - 8)),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.55, color, 2)
                    cv2.circle(out_frame, (gx, gy), 5, color, -1)  # ground point dot

                out_frame = draw_detections(frame, dets, cfg.class_names, args.score_thres)
                cv2.putText(
                    out_frame,
                    f"FPS: {fps:.1f}",
                    (10, 30),
